$(document).on("click", ".show_rate", function show_rate() {
...
$(document).on("click", ".add_comment", function add_comment(event) {
...
$(document).on("click", ".img_parent", function() {
    let src = $(this).attr("src");
    $("#imgZoom").modal("show");
    $("#zoom").attr("src", src);
});
...
$(document).on("click", ".fa-chevron-right", function() {
    let srcThum = $(".imgThum");
    let srcZoom = $("#zoom").attr("src");
    for (let i = 0; i < srcThum.length; i++) {
        let src = $(srcThum[i]).attr("src");
        if (src == srcZoom) {
            let a = i + 1;
            if (a > 4) {
                a = 0;
            }
            let sc = $(srcThum[a]).attr("src");
            $("#zoom").attr("src", sc);
        }
    }
});
