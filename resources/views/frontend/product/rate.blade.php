    ...
    <div class="rate_c_center">
        ...
        @foreach($arrayRatings as $key => $abc)
            <div class="main-item">
                ...
                <div class="row_star">
                    <div class="zero" style="width:{{($abc['total'] / count($product->Rate) *100)}}%;height:100%;background-color: #d4020d;"></div>
                </div>
                <label for="">{{ number_format(($abc['total'] / count($product->Rate) *100))}}%</label>
            </div>
        @endforeach
        @else
    ...
    <div class="rate">
        ...
        <a href="#"
           title="Thêm vào giỏ hàng"
            class="add_rate"
            data-url="{{route('product.rate',$product->id)}}"
        >
                            Gửi
        </a>
